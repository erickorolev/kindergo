...
//SalesPlatform.ru begin
function vtws_getGlobalSearchField ($moduleName) {
    global $adb;
    $moduleSearchSettings = 'SELECT * FROM berli_globalsearch_settings WHERE gstabid = ?';
    $result = $adb->pquery($moduleSearchSettings, array(getTabid($moduleName)));
    $resultRow = $adb->fetchByAssoc($result);
    if (isset($resultRow['displayfield']) && !empty($resultRow['displayfield'])) {
        $fields = explode(',', $resultRow['displayfield']);
    } else {
        $fields = vtws_getEntityNameFields($moduleName); 
    }
    
    if ($moduleName == 'Documents') {
        foreach ($fields as $key => $value) {
            if ($value == 'title') {
                $fields[$key] = 'notes_title';
            }
        } 
    }
