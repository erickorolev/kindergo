...
if ($module) {
    ...
    if ($block) {
        $field = Vtiger_Field::getInstance('trip_images_list', $module); // Название поля без пробелов через нижнее подчеркивание
        if (!$field) {
            $field               = new Vtiger_Field();
            $field->name         = 'trip_images_list';  // Название поля без пробелов через нижнее подчеркивание
            $field->table        = $module->basetable;
            $field->label        = 'LBL_TRIP_IMAGES_LIST';  // Лейбл на английском. Переводить на русский через файлы-переводов.
            $field->column       = 'trip_images_list';  // Название поля без пробелов через нижнее подчеркивание
