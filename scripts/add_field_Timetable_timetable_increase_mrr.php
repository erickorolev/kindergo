...
if ($module) {
    ...
    if ($block) {
        $field = Vtiger_Field::getInstance('timetable_increase_mrr', $module); // Название поля без пробелов через нижнее подчеркивание
        if (!$field) {
            $field               = new Vtiger_Field();
            $field->name         = 'timetable_increase_mrr';  // Название поля без пробелов через нижнее подчеркивание
            $field->table        = $module->basetable;
            $field->label        = 'LBL_TIMETABLE_INCREASE_MRR';  // Лейбл на английском. Переводить на русский через файлы-переводов.
            $field->column       = 'timetable_increase_mrr';  // Название поля без пробелов через нижнее подчеркивание
            $field->columntype   = 'DECIMAL(7,3)';  // Посмотреть тип у похожих полей в таблице vtiger_навание-модуля
