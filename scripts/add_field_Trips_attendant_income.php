...
$module = Vtiger_Module::getInstance('Trips'); // Имя модуля из таблицы vtiger_tab
if ($module) {
    $block = Vtiger_Block::getInstance('LBL_TRIPS_INFORMATION', $module); // Название блока из таблицы vtiger_blocks
    if ($block) {
        $field = Vtiger_Field::getInstance('attendant_income', $module); // Название поля без пробелов через нижнее подчеркивание
        if (!$field) {
            $field               = new Vtiger_Field();
            $field->name         = 'attendant_income';  // Название поля без пробелов через нижнее подчеркивание
            $field->table        = $module->basetable;
            $field->label        = 'LBL_ATTENDANT_INCOME';  // Лейбл на английском. Переводить на русский через файлы-переводов.
            $field->column       = 'attendant_income';  // Название поля без пробелов через нижнее подчеркивание
            $field->columntype   = 'DECIMAL(25,8)';  // Посмотреть тип у похожих полей в таблице vtiger_навание-модуля
            ...
            $field->generatedtype = 1; // Посмотреть тип у похожих полей в таблице vtiger_field
            ...
            //$pickListValues = array('Logical', 'Physical');
