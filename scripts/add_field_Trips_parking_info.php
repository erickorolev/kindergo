...
if ($module) {
    ...
    if ($block) {
        $field = Vtiger_Field::getInstance('parking_info', $module); // Название поля без пробелов через нижнее подчеркивание
        if (!$field) {
            $field               = new Vtiger_Field();
            $field->name         = 'parking_info';  // Название поля без пробелов через нижнее подчеркивание
            $field->table        = $module->basetable;
            $field->label        = 'LBL_PARKING_INFO';  // Лейбл на английском. Переводить на русский через файлы-переводов.
            $field->column       = 'parking_info';  // Название поля без пробелов через нижнее подчеркивание
            $field->columntype   = 'VARCHAR(250)';  // Посмотреть тип у похожих полей в таблице vtiger_навание-модуля
