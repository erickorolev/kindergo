...
$module = Vtiger_Module::getInstance('Invoice'); // Имя модуля из таблицы vtiger_tab
if ($module) {
    $block = Vtiger_Block::getInstance('LBL_INVOICE_INFORMATION', $module); // Название блока из таблицы vtiger_blocks
    if ($block) {
        $field = Vtiger_Field::getInstance('online_payment', $module); // Название поля без пробелов через нижнее подчеркивание
        if (!$field) {
            $field               = new Vtiger_Field();
            $field->name         = 'online_payment';  // Название поля без пробелов через нижнее подчеркивание
            $field->table        = $module->basetable;
            $field->label        = 'LBL_ONLINE_PAYMENT';  // Лейбл на английском. Переводить на русский через файлы-переводов.
            $field->column       = 'online_payment';  // Название поля без пробелов через нижнее подчеркивание
