...
$module = Vtiger_Module::getInstance('SPPayments'); // Имя модуля из таблицы vtiger_tab
if ($module) {
    $block = Vtiger_Block::getInstance('LBL_PAYMENT_DETAILS', $module); // Название блока из таблицы vtiger_blocks
    if ($block) {
        $field = Vtiger_Field::getInstance('attendanta_signature', $module); // Название поля без пробелов через нижнее подчеркивание
        if (!$field) {
            $field               = new Vtiger_Field();
            $field->name         = 'attendanta_signature';  // Название поля без пробелов через нижнее подчеркивание
            $field->table        = $module->basetable;
            $field->label        = 'LBL_ATTENDANTA_SIGNATURE';  // Лейбл на английском. Переводить на русский через файлы-переводов.
            $field->column       = 'attendanta_signature';  // Название поля без пробелов через нижнее подчеркивание
            ...
            $pickListValues = array('Signed by', 'Disputed');
