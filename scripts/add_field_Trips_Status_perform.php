...
if ($module) {
    ...
    if ($block) {
        $field = Vtiger_Field::getInstance('trips_status_perform', $module); // Название поля без пробелов через нижнее подчеркивание
        if (!$field) {
            $field               = new Vtiger_Field();
            $field->name         = 'trips_status_perform';  // Название поля без пробелов через нижнее подчеркивание
            $field->table        = $module->basetable;
            $field->label        = 'LBL_TRIPS_STATUS_PERFORM';  // Лейбл на английском. Переводить на русский через файлы-переводов.
            $field->column       = 'trips_status_perform';  // Название поля без пробелов через нижнее подчеркивание
            ...
            $pickListValues = array('Confirm', 'Late','In place', 'No child', 'Took child', 'Something happened', 'Brought child', 'Completed');
