...
if ($module) {
    ...
    if ($block) {
        $field = Vtiger_Field::getInstance('bill_paid', $module); // Название поля без пробелов через нижнее подчеркивание
        if (!$field) {
            $field               = new Vtiger_Field();
            $field->name         = 'bill_paid';  // Название поля без пробелов через нижнее подчеркивание
            $field->table        = $module->basetable;
            $field->label        = 'LBL_BILL_PAID';  // Лейбл на английском. Переводить на русский через файлы-переводов.
            $field->column       = 'bill_paid';  // Название поля без пробелов через нижнее подчеркивание
            $field->columntype   = 'VARCHAR(3)';  // Посмотреть тип у похожих полей в таблице vtiger_навание-модуля
