	...
	{else}
        <div class="fileUploadBtn btn btn-primary">
            ...
            <input type="file" class="inputElement {if $FIELD_MODEL->isNameField()}nameField{/if}" name="{$FIELD_MODEL->getFieldName()}"
                ...
                />
        </div>&nbsp;<span class="showFileName"></span>
		{if $IS_INTERNAL_LOCATION_TYPE && !empty($FIELD_VALUE)}
			&nbsp;[{$FIELD_VALUE}]
		{/if}
	{/if}
	<div class="uploadedFileDetails {if $IS_EXTERNAL_LOCATION_TYPE}hide{/if}" style="display:none;">
		...
		<div class="uploadFileSizeLimit redColor" style="display: none">
...
{literal}
	<script>
		function showFileName(fileInput) {
			...
			$(fileInput).closest('.fileUploadContainer').find('.showFileName').html(file_name);
			$(fileInput).closest('tr.relatedRecords').find('input[data-fieldname="Documents_notes_title"]').val(file_name);
