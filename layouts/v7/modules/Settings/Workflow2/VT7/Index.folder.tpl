...
<div class="container-fluid" id="moduleManagerContents">
    <div class="widget_header row-fluid">
        <div class="span6"><h3>{vtranslate('Workflow Designer', 'Workflow2')} &raquo; {vtranslate('Folderview', 'Settings:Workflow2')}</h3></div>
    </div>
    <hr>
    ...
    <div class="listViewContentDiv" id="listViewContents">
        ...
        {if $SHOW_CRON_NOTICE eq true}
            <div class="alert alert-error">
                <strong>{vtranslate('Please check your VtigerCRM cron setup! No cron was executed in last 24 hours. Otherwise you will get problems with some functions of Workflow Designer.', 'Settings:Workflow2')}</strong>&nbsp;&nbsp;&nbsp;<a class="btn" href="https://wiki.vtiger.com/index.php/Cron" target="_blank">{vtranslate('open','Settings:Workflow2')|ucfirst} {vtranslate('Information','Settings:Workflow2')}</strong></a>
            </div>
        {/if}
        ...
        <div class="row-fluid" style="margin-left:0;">
                            <span class='btn-toolbar span4'>
                                <button type="button" class="btn addButton addWorkflowButton"><i class="icon-plus"></i> <strong>{vtranslate("Create new workflow","Settings:Workflow2")}</strong></button>
                                <button type="button" class="btn addButton" onclick="importWorkflow();"><i class="icon-file"></i> <strong>{vtranslate("import Workflow","Settings:Workflow2")}</strong></button>
                            </span>
                            ...
                            <span class="btn-toolbar span5" style="display:flex;">
                                <select style="width:80%;" id="overviewFolder">
                                    ...
                                    {foreach from=$availFolder item=folder}
                                        <option value="{$folder|@urlencode}" {if ($targetFolder eq $folder)}selected='selected'{/if}>{$folder}</option>
                                    {/foreach}
                                </select>
                            ...
                            {if $is_admin eq true}
                                <button class="btn btn-success pull-right" onclick="window.location.href='index.php?module=Workflow2&view=Upgrade&parent=Settings';"><i class=" icon-white icon-asterisk"></i> <strong>{vtranslate("LBL_UPDATE_MODULE","Settings:Workflow2")}</strong></button>
                            {/if}
                            </span>
        </div>
        ...
        {foreach from=$workflows item=moduleArray key=folderName}
            <div data-folder="{$folderName}" class="WorkflowFolder {if $visibility[$folderName] eq '1'}FolderOpened{else}FolderClosed{/if}" style="{if !empty($folderSettings[$folderName])}background-color:{$folderSettings[$folderName]['color']};{/if}">
                <div class="WorkflowFolderTitle" style="text-transform:uppercase;">
                    ...
                    &nbsp;<span class="WorkflowFolderTitleContainer"><b>&nbsp;{$folderName}</b></span> ({$folderCounts[$folderName]}) <i class="icon-edit editFolderTitle"></i>
                </div>
