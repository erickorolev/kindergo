...
class Settings_Workflow2_FrontendManager_View extends Settings_Workflow2_Default_View {
    private $iconsFA = array("fa-500px","fa-address-book","fa-address-book-o","fa-address-card","fa-address-card-o","fa-adjust","fa-adn","fa-align-center","fa-align-justify","fa-align-left","fa-align-right",
	...
	public function process(Vtiger_Request $request) {
		...
		$availableFrontendTypes = \Workflow\FrontendTypes::getAllAvailable();
		$viewer->assign('FrontendTypes', $availableFrontendTypes);

        $sql = "SELECT * FROM vtiger_wf_settings WHERE module_name != '' ORDER BY module_name, active DESC, title";
        ...
        $sql = 'SELECT
                    ...
                    vtiger_wf_frontendmanager.module as module_name,
                    vtiger_wf_settings.title
                FROM vtiger_wf_frontendmanager
                    LEFT JOIN vtiger_wf_settings ON (vtiger_wf_settings.id = vtiger_wf_frontendmanager.workflow_id) ORDER BY vtiger_wf_frontendmanager.module, `order`
        ';
        $result = $adb->query($sql);
        ...
        while($row = $adb->fetchByAssoc($result)) {
            if(!empty($row['config'])) {
                ...
                $configurations[$row['id']] = array();
            }

            $links[getTranslatedString($row['module'], $row['module'])][] = $row;
        }

        $viewer->assign('links', $links);

        $sql = 'SELECT * FROM vtiger_wf_frontend_config';
        $result = $adb->query($sql);

        $frontendConfig = array();
        while($row = $adb->fetchByAssoc($result)) {
            $frontendConfig[$row['module']] = $row;
        }

        $viewer->assign('frontendConfig', $frontendConfig);
        $viewer->assign('configurations', $configurations);
        $viewer->assign('FA_ICONS', $this->iconsFA);

        $viewer->view('VT7/FrontendManager.tpl', $qualifiedModuleName);
	}
	...
	function getHeaderScripts(Vtiger_Request $request) {
		...
		$jsFileNames = array(
			...
			"modules.Settings.$moduleName.views.resources.FrontendManager",
			"modules.$moduleName.views.resources.js.jscolor.jscolor",
		);
