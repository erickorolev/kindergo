...
class VTEntity
{
    ...
    public function addTempFile($filepath, $filestoreid = null, $filename = null, $execID = false) {
        ...
        if(!file_exists($path . DIRECTORY_SEPARATOR . $recordId)) {
            @mkdir($path . DIRECTORY_SEPARATOR . $recordId . DIRECTORY_SEPARATOR, 0777, true);
        }
        ...
        if(file_exists($path . DIRECTORY_SEPARATOR . $recordId . DIRECTORY_SEPARATOR . $tmpid)) {
            ...
            $files[$filestoreid] = array('path' => $path . DIRECTORY_SEPARATOR . $recordId . DIRECTORY_SEPARATOR . $tmpid, 'name' => $filename, 'execid' => ($execID !== false ? $execID : \Workflow2::$currentBlockObj->getExecId()));

        } else {

            throw new \Exception('Workflow could not store file! ' . rtrim($root_directory, '/').'/modules/Workflow2/tmp/ must be writeable and existing!');
...
//                var_dump($focus->column_fields);
                ...
                if(($this->_moduleName == 'Products' || $this->_moduleName == 'Services') && !empty($this->_id)) {
                    $listPriceResult = $adb->pquery('SELECT * FROM vtiger_productcurrencyrel WHERE productid	= ?', array($this->_id));
    ...
    public function get($key) {
        ...
        if($key == 'lastComment')
            return \Workflow\VtUtils::getLastComment($this->_id);
        if($key == 'last5Comments')
            return \Workflow\VtUtils::getComments($this->_id, 5);
        if($key == 'comments')
            return \Workflow\VtUtils::getComments($this->_id);
    ...
    public function save() {
        ...
        try {
            ...
            foreach($changed as $key) {
                if(substr($key, -4) == '_raw' && isset($focus->column_fields[substr($key, 0, -4)])) continue;

                if($this->getModuleName() == 'Products' || $this->getModuleName() == 'Services') {
                    if (substr($key, 0, 7) == 'curname') continue;
                    if (substr($key, 0, 4) == 'cur_' && substr($key, -6) == '_check') continue;
                }

                //var_dump(substr($key, 0, 7));
                if(!in_array($key, array('record_id', 'record_module', 'label'))) {
