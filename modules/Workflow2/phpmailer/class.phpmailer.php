...
class Workflow_PHPMailer {
   ...
   * @param string $name
   * @throws Workflow_phpmailerException
  ...
  protected function AddAnAddress($kind, $address, $name = '') {
    if (!preg_match('/^(to|cc|bcc|Reply-To)$/', $kind)) {
      ...
      if ($this->exceptions) {
        throw new Workflow_phpmailerException('Invalid recipient array: ' . $kind);
      }
      if ($this->SMTPDebug) {
    ...
    if (!$this->ValidateAddress($address)) {
      ...
      if ($this->exceptions) {
        throw new Workflow_phpmailerException($this->Lang('invalid_address').': '.$address);
      }
      if ($this->SMTPDebug) {
  ...
  public function Send() {
    ...
    } catch (Workflow_phpmailerException $e) {
      $this->mailHeader = '';
  ...
  public function PreSend() {
    try {
      ...
      if ((count($this->to) + count($this->cc) + count($this->bcc)) < 1) {
        throw new Workflow_phpmailerException($this->Lang('provide_address'), self::STOP_CRITICAL);
      }
      ...
      if (!$this->AllowEmpty and empty($this->Body)) {
        throw new Workflow_phpmailerException($this->Lang('empty_message'), self::STOP_CRITICAL);
      }
    ...
    if ($this->SingleTo === true) {
      foreach ($this->SingleToArray as $val) {
        if(!@$mail = popen($sendmail, 'w')) {
          throw new Workflow_phpmailerException($this->Lang('execute') . $this->Sendmail, self::STOP_CRITICAL);
        }
        fputs($mail, "To: " . $val . "\n");
        fputs($mail, $header);
        fputs($mail, $body);
  ...
  protected function MailSend($header, $body) {
    ...
    if(!$rt) {
      throw new Workflow_phpmailerException($this->Lang('instantiate'), self::STOP_CRITICAL);
    }
    return true;
  }
  ...
  protected function SmtpSend($header, $body) {
    ...
    if(!$this->SmtpConnect()) {
      throw new Workflow_phpmailerException($this->Lang('smtp_connect_failed'), self::STOP_CRITICAL);
    }
    $smtp_from = ($this->Sender == '') ? $this->From : $this->Sender;
    if(!$this->smtp->Mail($smtp_from)) {
      $this->SetError($this->Lang('from_failed') . $smtp_from . ' : ' .implode(',', $this->smtp->getError()));
      throw new Workflow_phpmailerException($this->ErrorInfo, self::STOP_CRITICAL);
    }
    ...
    if (count($bad_rcpt) > 0 ) { //Create error message for any bad addresses
      ...
      throw new Workflow_phpmailerException($this->Lang('recipients_failed') . $badaddresses);
    }
    if(!$this->smtp->Data($header . $body)) {
      throw new Workflow_phpmailerException($this->Lang('data_not_accepted'), self::STOP_CRITICAL);
    }
        ...
        try {
          ...
          if ($tls) {
            if (!$this->smtp->StartTLS()) {
              throw new Workflow_phpmailerException($this->Lang('connect_host'));
            }
          ...
          if ($this->SMTPAuth) {
            if (!$this->smtp->Authenticate($this->Username, $this->Password, $this->AuthType, $this->Realm, $this->Workstation)) {
                throw new Workflow_phpmailerException($this->Lang('authenticate'));
            }
          }
          return true;
        } catch (Workflow_phpmailerException $e) {
  ...
  */
    ...
    if ($this->IsError()) {
      ...
      try {
        if (!defined('PKCS7_TEXT')) {
            throw new Workflow_phpmailerException($this->Lang('signing').' OpenSSL extension missing.');
        }
        ...
        if (@openssl_pkcs7_sign($file, $signed, 'file://'.realpath($this->sign_cert_file), array('file://'.realpath($this->sign_key_file), $this->sign_key_pass), null)) {
          ...
          @unlink($file);
          @unlink($signed);
          throw new Workflow_phpmailerException($this->Lang('signing').openssl_error_string());
        }
      } catch (Workflow_phpmailerException $e) {
        $body = '';
        if ($this->exceptions) {
  ...
  public function AddAttachment($path, $name = '', $encoding = 'base64', $type = '', $disposition = 'attachment') {
    try {
      if ( !@is_file($path) ) {
        throw new Workflow_phpmailerException($this->Lang('file_access') . $path, self::STOP_CONTINUE);
      }
  ...
  */
  ...
  protected function EncodeFile($path, $encoding = 'base64') {
    try {
      if (!is_readable($path)) {
        throw new Workflow_phpmailerException($this->Lang('file_open') . $path, self::STOP_CONTINUE);
      }
  ...
  public function set($name, $value = '') {
    try {
      if (isset($this->$name) ) {
        ...
        throw new Workflow_phpmailerException($this->Lang('variable_set') . $name, self::STOP_CRITICAL);
      }
    } catch (Exception $e) {
      $this->SetError($e->getMessage());
 ...
 * @package PHPMailer
 */
class Workflow_phpmailerException extends Exception {
  /**
