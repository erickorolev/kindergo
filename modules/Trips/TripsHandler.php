...
function calculateTripPay($entity)
{
	...
	if ($childrens==1)
	{
		$itogoPrice3=$Cl->getInvService(14,$duration);  //сопровождение одного ребенка
		$itogoPriceALL=$itogoPriceALL+$itogoPrice3;
	}
	elseif ($childrens==2)
	{
		$itogoPrice3=$Cl->getInvService(20,$duration);  //сопровождение одного ребенка
		$itogoPriceALL=$itogoPriceALL+$itogoPrice3;
	}
	elseif ($childrens==3)
	{
		$itogoPrice3=$Cl->getInvService(21,$duration);  //сопровождение одного ребенка
		$itogoPriceALL=$itogoPriceALL+$itogoPrice3;
	}
	elseif ($childrens==4)
	{
		$itogoPrice3=$Cl->getInvService(22,$duration);  //сопровождение одного ребенка
		$itogoPriceALL=$itogoPriceALL+$itogoPrice3;
	}
	else
	{
	...
	$itogoPay=$itogoPriceALL-$itogoPriceALL*0.35+$shedAll*0.56+$notShedPay*0.42+$parking_cost;
